// Google Apps Script để xử lý requests từ Laravel
// Copy code này vào Google Apps Script và deploy as web app

function doPost(e) {
  try {
    Logger.log('POST request received from Laravel');
...
function doGet(e) {
  try {
    ...
    return ContentService
      .createTextOutput('Laravel-Google Sheets integration is working!')
...
// Function to test Laravel integration
function testLaravelIntegration() {
  try {
    Logger.log('Testing Laravel integration');
    
    const testData = {
      type: 'confirm',
      timestamp: new Date().toISOString(),
      name: 'Test User from Laravel',
      phone: '0123456789',
      email: 'test@laravel.com',
      message: 'Test message from Laravel integration'
    };
    ...
    Logger.log('Laravel integration test successful');
  } catch (error) {
    Logger.log('Error in testLaravelIntegration: ' + error.toString());
  }
} 